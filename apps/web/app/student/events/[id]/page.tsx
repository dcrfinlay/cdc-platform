      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        {registered && (
          <div className="mb-6 px-4 py-3 rounded-lg bg-[#E1F5EE] border border-[#c2e8d8] text-[#0F6E56] text-[13px]">
            ✓ You&apos;re registered! Show your QR code at the event entrance.
          </div>
        )}
          ...
          <div className="p-6">
            {myReg ? (
              <div className="space-y-4">
                ...
                {myReg.attended_at && (
                  <p className="text-[12px] text-[#888]">Attendance confirmed ✓</p>
                )}
                {qrImageUrl && !myReg.attended_at && (
                  <div className="flex flex-col items-center gap-3 py-5 border border-[#e5e4df] rounded-xl">
                    <p className="text-[12px] text-[#666]">Show this QR code at the event entrance</p>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={qrImageUrl} alt="Attendance QR code" width={160} height={160} />
                    <p className="text-[10px] text-[#aaa]">Token: {myReg.qr_token.slice(0, 8)}…</p>
                  </div>
                )}
